...
const deleteClassesFromUser = async(id, userToDelete) => { 
    ...
    users.updateOne({_id: ObjectId(userToDelete).valueOf()}, {$pull: {"classes": id.toString()}})
...
const deleteClass = async(id) => { 
    ...
    postsArray.forEach(post => { //get rid of all posts for that class 
        posts.deletePost(post)
    })

    const studentArray = classes.students; //get rid of the class we are going to delete from all students
    studentArray.forEach(student => { 
        users.updateOne({_id: ObjectId(student).valueOf()}, {$pull: {"classes": id.toString()}})
    })
    users.updateOne({_id: ObjectID(classes.instructor).valueOf()}, {$pull: {"classes": id.toString()}}) //update for instructors
